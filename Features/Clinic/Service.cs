...
public class ClinicService (DatabaseContext _context) : IClinicService
{
    ...
    public async Task<IReadOnlyList<Dto.SpecializationDto>> GetSpecializationAsync(CancellationToken cancellationToken)
    {
        var specializations = await _context
            .Specializations
            .AsNoTracking()
            .OrderBy(x => x.Name)
            .Select(x => new Dto.SpecializationDto(x.SpecializationId, x.Name, x.Description))
            .ToListAsync(cancellationToken);

        return specializations;
    }

    public async Task<IReadOnlyList<Dto.SpecializationDto>?> GetSpecializationByCityAsync(int cityId, CancellationToken cancellationToken)
    {
        ...
        return specializations.Select(x => new Dto.SpecializationDto(
                x.SpecializationId,
                x.Name,
                x.Description))
            .ToList();
