...
public class DoctorService : IDoctorService
{
    ...
    public async Task<DoctorScheduleDto?> UpsertMyScheduleAsync(string userId, UpsertDoctorScheduleDto request, CancellationToken cancellationToken)
    {
        ...
        var membershipExists = await _dbContext.ClinicDoctors.AnyAsync(
            x => x.ClinicId == request.ClinicId && x.DoctorId == doctor.DoctorId,
            cancellationToken
        );
        ...
        if (request.ScheduleId.HasValue)
        {
            schedule = await _dbContext.DoctorSchedules.FirstOrDefaultAsync(
                x => x.ScheduleId == request.ScheduleId.Value && x.DoctorId == doctor.DoctorId,
                cancellationToken
    ...
    public async Task<bool> DeleteMyScheduleAsync(string userId, int scheduleId, CancellationToken cancellationToken)
    {
        ...
        var schedule = await _dbContext.DoctorSchedules.FirstOrDefaultAsync(
            x => x.ScheduleId == scheduleId && x.DoctorId == doctor.DoctorId,
            cancellationToken
        );
    ...
    )
    {
        ...
        var bookedIntervals = doctor.Appointments
            ...
            .Where(x => x is not null && x!.Value.Date == date)
            .Select(x => x!.Value)
            .ToList();

        var orderedSlots = BuildAvailableSlots(doctorId, date, appointmentType, clinicId, doctor.DoctorSchedules, bookedIntervals);
    ...
    )
    {
        ...
        for (var day = 1; day <= daysInMonth; day++)
        {
            ...
            var slots = BuildAvailableSlots(doctorId, date, appointmentType, clinicId, doctor.DoctorSchedules, bookedIntervals);
        ...
        return new DoctorAvailabilityCalendarDto(
            doctorId,
            year,
            month,
            appointmentTypeId,
    ...
    private async Task<Doctor?> LoadAvailabilityDoctorAsync(int doctorId, int? clinicId, CancellationToken cancellationToken)
    {
        ...
        return await query.FirstOrDefaultAsync(x => x.DoctorId == doctorId, cancellationToken);
    }
    ...
    private static IReadOnlyList<DoctorAvailabilitySlotDto> BuildAvailableSlots(
        ...
        IReadOnlyList<(DateOnly Date, DateTime Start, DateTime End)> bookedIntervals
    )
    {
        var targetDayOfWeek = AppointmentSchedulingHelper.NormalizeDayOfWeek((int)date.DayOfWeek);
        var slots = new Dictionary<int, DoctorAvailabilitySlotDto>();
        ...
        foreach (var schedule in schedules)
        {
            ...
            if (clinicId.HasValue && schedule.ClinicId != clinicId.Value)
            {
                continue;
            }
            ...
            if (validFrom > date || (validTo.HasValue && validTo.Value < date))
            {
                continue;
            }
            ...
            while (currentStart.AddMinutes(appointmentType.DurationMinutes) <= scheduleEnd)
            {
                ...
                var timeSlotId = AppointmentSchedulingHelper.BuildTimeSlotId(doctorId, date, currentStart);
                slots[timeSlotId] = new DoctorAvailabilitySlotDto(
        ...
        return slots.Values
            .OrderBy(x => x.StartAt, StringComparer.Ordinal)
            .ToList();
    }
    ...
    private static DoctorProfileDto MapProfile(Doctor doctor)
    {
        ...
        return new DoctorProfileDto(
            ...
            doctor.DoctorSpecializations
                ...
                .OrderBy(x => x)
                .ToList(),
            doctor.DoctorAppointmentTypes
                .Select(x => MapAppointmentType(x.AppointmentType))
    ...
    private static DoctorClinicDto MapClinic(ClinicDoctor clinicDoctor)
    {
        return new DoctorClinicDto(
            clinicDoctor.ClinicId,
            clinicDoctor.Clinic.Name,
            clinicDoctor.Clinic.City.Name,
            clinicDoctor.Clinic.StreetAddress
        );
    }
    ...
    {
        return new DoctorAppointmentTypeDto(
    ...
    private static (DateOnly Date, DateTime Start, DateTime End)? BuildBookedInterval(Appointment.Appointment appointment)
    {
        try
        {
